...
const StartProject = () => {
    ...
    return (
        <div className=' relative w-full p-32 bg-zinc-800'>
            <div className=' absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-10'>
                    <div className=' flex items-center justify-center h-[15vw] w-[15vw] bg-zinc-900 rounded-full'>
                        <div style={{ transform: `translate(${moveEye[0]}%, ${moveEye[1]}%)` }} className='relative h-2/3 w-2/3 bg-zinc-100 rounded-full'>
                            <div style={{ transform: `translate(-50%, -50%) rotate(${rotate}deg)` }} className='w-full relative top-1/2 left-1/2'>
                                <div className='h-6 w-6 bg-zinc-900 rounded-full'></div>
                            </div>
                        </div>
                    </div>
                    <div className=' flex items-center justify-center h-[15vw] w-[15vw] bg-zinc-900 rounded-full'>
            ...
            <div className="headings w-full flex flex-col items-center">
                {["ready", "to start", "the project?"].map((item, index) => (
                    <h1 key={index} className='text-[12vw] text-zinc-100 leading-none tracking-tighter uppercase'>{item}</h1>
                ))}
            </div>
